#This code generates a PCA plot for experimental data and distance between cluster centres
#Based on the seeds set for clustering, for the given datasets, cluster number 1 is melanocytic, 
#2 is transitory, 3 in NCSC and 4 is undifferentiated
...
#Calculate pairwise euclidean distance between cluster centres
...
#Determine closest cluster center from melanocytic and undifferentiated phenotypes
...
#Determines the distance of melanocytic phenotype from the rest
dist_ord <- function(d){
...
dist_PCA_plot <- function(data, cut, list){
  ...
  mean <- mean_PCA(data,cut$cluster)         #Identify cluster centres on PC1 
